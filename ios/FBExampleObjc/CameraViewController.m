 ...
 */
@interface CameraViewController () <UIImagePickerControllerDelegate, UINavigationControllerDelegate, PHPickerViewControllerDelegate>
...
- (void)cameraManager:(id)cameraManager didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    // 通过引擎处理帧（视频模式）
    if (inputImage) {
        FBImageFrame *outputFrame = [self.beautyEffectEngine processImage:inputImage processMode:FBProcessModeVideo];
        
        // 提取 RGBA 并渲染到 OpenGL 视图
        if (outputFrame) {
            FBImageBuffer *rgbaBuffer = [outputFrame toRGBA];
            if (rgbaBuffer) {
                [self.previewView renderBuffer:rgbaBuffer];
            }
        }
    }
        ...
        if ([tab isEqualToString:@"beauty"]) {
            ...
            if ([function isEqualToString:@"none"]) {
                ...
                if (presetImage) {
                    ...
                    NSString *imagePath = [[NSBundle mainBundle] pathForResource:@"back_mobile" ofType:@"jpg"];
                    if (!imagePath) {
                ...
                // 背景图片切换（从相册选择）
                // 注意：当用户点击 "image" 按钮时，会触发 beautyPanelDidRequestImageSelection:function: 方法
                // 这里不需要处理，因为图片选择是异步的，会在 handleSelectedImage: 中处理
            }
        }
...
- (void)showPhotoLibraryPermissionAlert {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"相册权限"
...
- (void)showImageProcessingErrorAlert {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"处理失败"
