...
- (void)setupTopBar {
    ...
    [self.galleryButton addTarget:self action:@selector(galleryButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.flipCameraButton addTarget:self action:@selector(flipCameraButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setupPanel {
    ...
    self.panelRootView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.8]; // 半透明黑色背景
...
- (void)setupTabScrollView {
    ...
    // Tab 按钮
    NSArray *tabs = @[@"美颜", @"美型", @"美妆", @"滤镜", @"贴纸", @"美体", @"虚拟背景", @"画质调整"];
    NSMutableArray *buttons = [[NSMutableArray alloc] init];
    
    for (NSInteger i = 0; i < tabs.count; i++) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        [button setTitle:tabs[i] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor colorWithRed:0.67 green:0.67 blue:0.67 alpha:1.0] forState:UIControlStateNormal];
...
- (void)setupBottomButtonContainer {
    ...
    self.captureButton.backgroundColor = [UIColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:1.0]; // #00FF00
    ...
    [self.hidePanelButton addTarget:self action:@selector(hidePanelButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
    [buttonStack addArrangedSubview:self.hidePanelButton];
    
    // 约束
    [NSLayoutConstraint activateConstraints:@[
        [buttonStack.topAnchor constraintEqualToAnchor:self.bottomButtonContainer.topAnchor constant:10],
        [buttonStack.leadingAnchor constraintEqualToAnchor:self.bottomButtonContainer.leadingAnchor constant:16],
        [buttonStack.trailingAnchor constraintEqualToAnchor:self.bottomButtonContainer.trailingAnchor constant:-16],
        [buttonStack.bottomAnchor constraintEqualToAnchor:self.bottomButtonContainer.bottomAnchor constant:-10]
    ]];
}
...
- (void)setupPanelConstraints {
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.bottomButtonContainer.leadingAnchor constraintEqualToAnchor:self.panelRootView.leadingAnchor],
        [self.bottomButtonContainer.trailingAnchor constraintEqualToAnchor:self.panelRootView.trailingAnchor],
        ...
        [self.functionScrollView.leadingAnchor constraintEqualToAnchor:self.panelRootView.leadingAnchor constant:16],
        [self.functionScrollView.trailingAnchor constraintEqualToAnchor:self.panelRootView.trailingAnchor constant:-16],
        [self.functionScrollView.bottomAnchor constraintEqualToAnchor:self.bottomButtonContainer.topAnchor],
        [self.functionScrollView.heightAnchor constraintEqualToConstant:120],
        
        // 功能按钮容器
        [self.functionButtonContainer.topAnchor constraintEqualToAnchor:self.functionScrollView.topAnchor],
        [self.functionButtonContainer.leadingAnchor constraintEqualToAnchor:self.functionScrollView.leadingAnchor],
        [self.functionButtonContainer.trailingAnchor constraintEqualToAnchor:self.functionScrollView.trailingAnchor],
        [self.functionButtonContainer.bottomAnchor constraintEqualToAnchor:self.functionScrollView.bottomAnchor],
        [self.functionButtonContainer.heightAnchor constraintEqualToAnchor:self.functionScrollView.heightAnchor],
        ...
        [self.tabScrollView.topAnchor constraintEqualToAnchor:self.panelRootView.topAnchor], // 面板从 Tab 区域开始
        ...
        [self.tabContainer.leadingAnchor constraintEqualToAnchor:self.tabScrollView.leadingAnchor constant:8],
        [self.tabContainer.trailingAnchor constraintEqualToAnchor:self.tabScrollView.trailingAnchor constant:-8],
        [self.tabContainer.heightAnchor constraintEqualToConstant:34], // 50 - 8*2 = 34 (Tab ScrollView 高度 50 - 上下 padding 8*2)
        
        // 子选项滚动视图（与功能按钮重叠，显示时替换功能按钮）
        [self.subOptionScrollView.leadingAnchor constraintEqualToAnchor:self.panelRootView.leadingAnchor],
        [self.subOptionScrollView.trailingAnchor constraintEqualToAnchor:self.panelRootView.trailingAnchor],
        [self.subOptionScrollView.bottomAnchor constraintEqualToAnchor:self.bottomButtonContainer.topAnchor],
        ...
        [self.subOptionContainer.leadingAnchor constraintEqualToAnchor:self.subOptionScrollView.leadingAnchor constant:16],
        [self.subOptionContainer.bottomAnchor constraintEqualToAnchor:self.subOptionScrollView.bottomAnchor],
        [self.subOptionContainer.heightAnchor constraintEqualToAnchor:self.subOptionScrollView.heightAnchor]
    ]];
    
    // 设置子选项容器宽度约束：使用 >= 关系，允许内容超出 ScrollView 宽度
    // 这样 StackView 可以根据内容自动扩展，ScrollView 才能滚动
    NSLayoutConstraint *subOptionWidthConstraint = [self.subOptionContainer.widthAnchor constraintGreaterThanOrEqualToAnchor:self.subOptionScrollView.widthAnchor constant:-32];
    ...
    self.sliderContainerHeightConstraint = [self.sliderContainer.heightAnchor constraintEqualToConstant:0];
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.beforeAfterButton.trailingAnchor constraintEqualToAnchor:self.view.trailingAnchor constant:-16]
    ]];
...
- (void)setupBottomControlPanel {
    ...
    for (NSDictionary *config in buttonConfigs) {
        ...
        if ([config[@"title"] isEqualToString:@"拍照"]) {
            ...
            innerCircleView.backgroundColor = [UIColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:1.0]; // #00FF00
            ...
            // 拍照按钮点击事件
            [button addTarget:self action:@selector(captureButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
        } else {
            // 其他按钮：根据标题映射到对应的 Tab
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.bottomControlPanel.bottomAnchor constraintEqualToAnchor:self.view.safeAreaLayoutGuide.bottomAnchor],
        ...
        [buttonStack.bottomAnchor constraintEqualToAnchor:self.bottomControlPanel.bottomAnchor constant:-16]
    ]];
}
...
- (void)showPanel {
    ...
    self.beforeAfterButtonBottomConstraint = [self.beforeAfterButton.bottomAnchor constraintEqualToAnchor:self.panelRootView.topAnchor constant:-10];
...
- (void)hidePanel {
    ...
    self.beforeAfterButtonBottomConstraint = [self.beforeAfterButton.bottomAnchor constraintEqualToAnchor:self.bottomControlPanel.topAnchor constant:-10];
    self.beforeAfterButtonBottomConstraint.active = YES;
...
- (UIButton *)createFunctionButton:(NSDictionary *)function {
    ...
    indicator.backgroundColor = [UIColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:1.0]; // #00FF00 绿色
    ...
    NSString *fullKey = [NSString stringWithFormat:@"%@:%@", self.currentTab ?: @"", functionKey ?: @""];
...
- (NSInteger)tabIndexForName:(NSString *)tabName {
    NSArray *tabNames = @[@"beauty", @"reshape", @"makeup", @"filter", @"sticker", @"body", @"virtual_bg", @"quality"];
...
- (void)setupSliderContainer {
    ...
    [NSLayoutConstraint activateConstraints:@[
        [self.valueLabel.leadingAnchor constraintEqualToAnchor:self.sliderContainer.leadingAnchor constant:16],
        ...
        [self.valueSlider.leadingAnchor constraintEqualToAnchor:self.sliderContainer.leadingAnchor constant:16],
        [self.valueSlider.trailingAnchor constraintEqualToAnchor:self.sliderContainer.trailingAnchor constant:-80], // 为 Before/After 按钮留出空间
    ...
    [self.beforeAfterButton addTarget:self action:@selector(beforeAfterButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
  // 按住预览原图：按下关闭所有参数，松开恢复用户已设置参数
  [self.beforeAfterButton addTarget:self action:@selector(beforeAfterTouchDown:) forControlEvents:UIControlEventTouchDown];
  [self.beforeAfterButton addTarget:self action:@selector(beforeAfterTouchUp:) forControlEvents:(UIControlEventTouchUpInside | UIControlEventTouchUpOutside | UIControlEventTouchCancel)];
...
- (void)sliderValueChanged:(UISlider *)sender {
    ...
    if (self.currentTab && self.currentFunction && [self.delegate respondsToSelector:@selector(beautyPanelDidChangeParam:function:value:)]) {
...
- (void)handleToggleFunction:(NSString *)functionKey {
    ...
    if ([functionKey isEqualToString:@"image"] && [self.currentTab isEqualToString:@"virtual_bg"]) {
        // 图像按钮：打开图片选择器
        if ([self.delegate respondsToSelector:@selector(beautyPanelDidRequestImageSelection:function:)]) {
    ...
    if ([self.delegate respondsToSelector:@selector(beautyPanelDidChangeParam:function:value:)]) {
        [self.delegate beautyPanelDidChangeParam:self.currentTab function:functionKey value:newState ? 1.0f : 0.0f];
    }
...
- (void)updateSelectionIndicators {
    ...
    NSString *selectedKey = [NSString stringWithFormat:@"%@:%@", self.currentTab ?: @"", self.currentFunction ?: @""];
