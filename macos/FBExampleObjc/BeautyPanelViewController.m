...
- (void)mouseDown:(NSEvent *)event {
    ...
    if (self.panelController && [self.panelController respondsToSelector:@selector(beforeAfterTouchDown)]) {
...
- (void)mouseUp:(NSEvent *)event {
    ...
    if (self.panelController && [self.panelController respondsToSelector:@selector(beforeAfterTouchUp)]) {
...
- (void)updateScrollViewContentSize {
    ...
    if (self.functionButtons.count > 0) {
        ...
        CGFloat padding = 0;
        CGFloat totalWidth = self.functionButtons.count * buttonWidth + (self.functionButtons.count - 1) * spacing + padding * 2;
...
- (void)setupTabScrollView {
    ...
    // NSScrollView 的文档视图应该使用 frame-based 布局，不需要设置 translatesAutoresizingMaskIntoConstraints
    [self.tabScrollView setDocumentView:self.tabContainer];
    
    // Tab 按钮
    NSArray<NSString *> *tabs = @[@"美颜", @"美型", @"美妆", @"滤镜", @"贴纸", @"美体", @"虚拟背景", @"画质调整"];
    ...
    for (NSInteger i = 0; i < tabs.count; i++) {
        ...
        // 设置文字颜色
        NSMutableAttributedString *attributedTitle = [[NSMutableAttributedString alloc] initWithString:tabTitle];
        if (i == 0) {
            // 第一个默认选中
            [attributedTitle addAttribute:NSForegroundColorAttributeName value:[NSColor whiteColor] range:NSMakeRange(0, tabTitle.length)];
            [attributedTitle addAttribute:NSFontAttributeName value:[NSFont systemFontOfSize:16 weight:NSFontWeightMedium] range:NSMakeRange(0, tabTitle.length)];
        } else {
            [attributedTitle addAttribute:NSForegroundColorAttributeName value:[NSColor colorWithRed:0.67 green:0.67 blue:0.67 alpha:1.0] range:NSMakeRange(0, tabTitle.length)];
            [attributedTitle addAttribute:NSFontAttributeName value:[NSFont systemFontOfSize:16] range:NSMakeRange(0, tabTitle.length)];
        }
...
- (void)setupTabButtonConstraints {
    ...
    [NSLayoutConstraint activateConstraints:@[
        [firstButton.leadingAnchor constraintEqualToAnchor:self.tabContainer.leadingAnchor constant:padding],
    ...
    NSLayoutConstraint *trailingConstraint = [lastButton.trailingAnchor constraintEqualToAnchor:self.tabContainer.trailingAnchor constant:-padding];
...
- (NSButton *)createFunctionButton:(NSDictionary *)function {
    ...
    indicator.layer.backgroundColor = [[NSColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:1.0] CGColor];
    ...
    NSLog(@"[BeautyPanel] Created function button: %@, tag: %ld", function[@"title"], (long)[self.functionButtons count]);
...
- (void)setupFunctionButtonConstraints {
    ...
    firstButton.translatesAutoresizingMaskIntoConstraints = NO;
            [NSLayoutConstraint activateConstraints:@[
        [firstButton.leadingAnchor constraintEqualToAnchor:self.functionButtonContainer.leadingAnchor constant:padding],
...
- (void)setupBottomButtonContainer {
    ...
    if (!resetIcon) {
        // 使用系统图标
        if (@available(macOS 11.0, *)) {
            resetIcon = [NSImage imageWithSystemSymbolName:@"arrow.counterclockwise" accessibilityDescription:nil];
        } else {
            // macOS 10.x 创建占位图标
    ...
    self.captureButton.layer.backgroundColor = [[NSColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:1.0] CGColor];
    ...
    NSLog(@"[BeautyPanel] hidePanelButton created, enabled: %d, target: %@, action: %s", 
          self.hidePanelButton.enabled, self.hidePanelButton.target, sel_getName(self.hidePanelButton.action));
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [buttonStack.bottomAnchor constraintEqualToAnchor:self.bottomButtonContainer.bottomAnchor constant:-5],
        ...
        [captureButtonContainer.widthAnchor constraintEqualToAnchor:buttonStack.widthAnchor multiplier:0.33],
        ...
        [self.hidePanelButton.widthAnchor constraintEqualToAnchor:buttonStack.widthAnchor multiplier:0.33],
...
- (void)setupPanelConstraints {
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.beforeAfterButton.trailingAnchor constraintEqualToAnchor:self.sliderContainer.trailingAnchor constant:-16],
        [self.beforeAfterButton.centerYAnchor constraintEqualToAnchor:self.sliderContainer.centerYAnchor]
    ]];
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.functionScrollView.topAnchor constraintEqualToAnchor:self.tabScrollView.bottomAnchor constant:12],
        ...
        [self.bottomButtonContainer.topAnchor constraintEqualToAnchor:self.functionScrollView.bottomAnchor constant:8],
        [self.bottomButtonContainer.bottomAnchor constraintEqualToAnchor:self.panelRootView.bottomAnchor],
...
- (void)selectTabButton:(NSInteger)index {
    for (NSInteger i = 0; i < self.tabButtons.count; i++) {
        ...
        if (i == index) {
            ...
            [attributedTitle addAttribute:NSFontAttributeName value:[NSFont systemFontOfSize:16 weight:NSFontWeightMedium] range:NSMakeRange(0, title.length)];
        } else {
            [attributedTitle addAttribute:NSForegroundColorAttributeName value:[NSColor colorWithRed:0.67 green:0.67 blue:0.67 alpha:1.0] range:NSMakeRange(0, title.length)];
            [attributedTitle addAttribute:NSFontAttributeName value:[NSFont systemFontOfSize:16] range:NSMakeRange(0, title.length)];
        }
            [button setAttributedTitle:attributedTitle];
        }
    }
...
- (void)functionButtonTapped:(NSButton *)sender {
    ...
    if (sender.tag < self.functionButtons.count) {
        ...
        if (sender.tag < functions.count) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(beautyPanelDidChangeParam:function:value:)]) {
            ...
            NSLog(@"  - ERROR: function index %ld out of range (count: %lu)", (long)sender.tag, (unsigned long)functions.count);
        }
    } else {
        NSLog(@"  - ERROR: button tag %ld out of range (count: %lu)", (long)sender.tag, (unsigned long)self.functionButtons.count);
    }
}
...
- (void)showPanel {
    ...
    // 通知父视图控制器更新布局约束
    [[NSNotificationCenter defaultCenter] postNotificationName:@"BeautyPanelVisibilityChanged" object:@YES];
}
...
- (void)hidePanel {
    ...
    // 通知父视图控制器更新布局约束
    [[NSNotificationCenter defaultCenter] postNotificationName:@"BeautyPanelVisibilityChanged" object:@NO];
}
...
- (void)setupSliderContainer {
    ...
    self.beforeAfterButton.layer.backgroundColor = [[NSColor colorWithRed:0.3 green:0.3 blue:0.3 alpha:1.0] CGColor];
    ...
    if (!beforeAfterIcon) {
        if (@available(macOS 11.0, *)) {
            beforeAfterIcon = [NSImage imageWithSystemSymbolName:@"arrow.left.arrow.right" accessibilityDescription:nil];
        }
    }
...
- (void)sliderValueChanged:(NSSlider *)sender {
    ...
    if (self.currentTab && self.currentFunction && self.delegate && [self.delegate respondsToSelector:@selector(beautyPanelDidChangeParam:function:value:)]) {
...
- (void)beforeAfterTouchUp {
    ...
    for (NSString *key in self.functionProgress.allKeys) {
        ...
        if (components.count == 2) {
            ...
            if (progress && self.delegate && [self.delegate respondsToSelector:@selector(beautyPanelDidChangeParam:function:value:)]) {
