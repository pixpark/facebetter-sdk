...
- (void)setupGL {
  ...
  glViewport(0,
             0,
             (GLsizei)(self.bounds.size.width * self.contentScaleFactor),
...
- (void)setMirrored:(BOOL)mirrored {
  _mirrored = mirrored;
  [EAGLContext setCurrentContext:_context];
  GLfloat texCoords[] = {_mirrored ? 1.0f : 0.0f,
                         1.0f,
                         _mirrored ? 0.0f : 1.0f,
                         1.0f,
                         _mirrored ? 1.0f : 0.0f,
...
- (void)updateTexture:(FBImageFrame *)frame {
  ...
  if (stride == width * 4) {
    ...
    for (int y = 0; y < height; y++) {
      glTexSubImage2D(
          GL_TEXTURE_2D, 0, 0, y, width, 1, GL_RGBA, GL_UNSIGNED_BYTE, data + y * stride);
    }
  }
}
...
- (void)renderFrame:(FBImageFrame *)frame {
  ...
  [_bufferLock lock];
  // 仅记录当前帧及尺寸，具体顶点更新放在 display，确保以最新视口尺寸计算
...
- (void)display {
  ...
  [_bufferLock lock];
  FBImageFrame *frame = _currentFrame;
  [_bufferLock unlock];
